...
from game import GameState, Position, Wall, get_save_files, load_game, save_game
...
def load_ai_player(player: int, difficulty: str = "medium"):
    ...
    if checkpoint_path and os.path.exists(checkpoint_path):
        ...
        print(
            f"Difficulty: {difficulty.upper()} (sims={num_simulations}, temp={temperature})"
        )
        try:
            ...
            if "Missing key(s)" in str(e) or "Unexpected key(s)" in str(e):
                print(
                    "Warning: Checkpoint architecture mismatch (old model has different NUM_RES_BLOCKS)"
                )
...
def rescale_assets():
    ...
    tile_img = pygame.transform.scale(
        raw_tile_img, (layout.tile_size, layout.tile_size)
    )
...
def get_tile_center(row: int, col: int) -> tuple[int, int]:
    ...
    x = (
        layout.margin_x
        + (col * (layout.tile_size + layout.gap_size))
        + layout.tile_size // 2
    )
    y = (
        layout.margin_y
        + (row * (layout.tile_size + layout.gap_size))
        + layout.tile_size // 2
    )
    return (x, y)
...
def get_wall_rect(wall: Wall) -> pygame.Rect:
    ...
    if orientation == "H":
        ...
        y = (
            ...
            + row * (layout.tile_size + layout.gap_size)
            + layout.tile_size
            + (layout.gap_size - layout.wall_thickness) // 2
        )
...
def draw_button(
    ...
    if disabled:
        ...
        border_color = (
            s["border"]
            if hover
            else (s["border"][0] // 2, s["border"][1] // 2, s["border"][2] // 2)
        )
        text_color = s["text"]
...
def draw_board(
    ...
    wall_preview: Wall | None = None,
    wall_valid: bool = True,
) -> None:
    ...
    for pos in valid_moves:
        ...
        glow = pygame.Surface(
            (layout.tile_size + glow_size * 2, layout.tile_size + glow_size * 2),
            pygame.SRCALPHA,
        )
        pygame.draw.rect(
            glow,
            (*HIGHLIGHT_COLOR, int(40 + 30 * pulse)),
            glow.get_rect(),
            border_radius=12,
        )
        ...
        highlight = pygame.Surface(
            (layout.tile_size, layout.tile_size), pygame.SRCALPHA
        )
        alpha = int(80 + 40 * pulse)
        pygame.draw.rect(
            highlight, (*HIGHLIGHT_COLOR, alpha), highlight.get_rect(), border_radius=8
        )
        pygame.draw.rect(
            highlight,
            (*HIGHLIGHT_PULSE, int(150 + 50 * pulse)),
            highlight.get_rect(),
            3,
            border_radius=8,
        )
    ...
    if wall_preview:
        ...
        pygame.draw.rect(
            preview_surf, (*color, 180), preview_surf.get_rect(), border_radius=3
        )
        pygame.draw.rect(
            preview_surf, (*color, 255), preview_surf.get_rect(), 2, border_radius=3
        )
    ...
    for center, color, glow_color, is_glowing in [
        ...
        if is_glowing:
            ...
            pygame.draw.circle(
                glow_surf,
                (*glow_color, int(60 + 30 * pulse)),
                (glow_radius + 10, glow_radius + 10),
                glow_radius,
            )
            screen.blit(
                glow_surf, (center[0] - glow_radius - 10, center[1] - glow_radius - 10)
            )

        # Shadow
        pygame.draw.circle(
            screen, (20, 15, 10), (center[0] + 3, center[1] + 3), layout.pawn_radius
        )
        ...
        highlight_pos = (
            center[0] - layout.pawn_radius // 3,
            center[1] - layout.pawn_radius // 3,
        )
        highlight_surf = pygame.Surface(
            (layout.pawn_radius * 2, layout.pawn_radius * 2), pygame.SRCALPHA
        )
        pygame.draw.circle(
            highlight_surf,
            (*[min(255, c + 60) for c in color], 100),
            (layout.pawn_radius // 2, layout.pawn_radius // 2),
            layout.pawn_radius // 2,
        )
        screen.blit(
            highlight_surf,
            (center[0] - layout.pawn_radius, center[1] - layout.pawn_radius),
        )
...
def draw_ui(
    ...
    screen.blit(
        ...
        (
            layout.margin_x + 30,
            top_panel.y + (panel_height - turn_surf.get_height()) // 2,
        ),
    )

    # Mode panel (top right)
    mode_panel_width = min(370, int(sw * 0.3))
    mode_panel = pygame.Rect(
        sw - mode_panel_width - 10, 5, mode_panel_width, panel_height
    )
    ...
    screen.blit(
        ...
        (
            mode_panel.x + 10,
            mode_panel.y + (panel_height - mode_surf.get_height()) // 2,
        ),
    )
    ...
    p1_surf = font.render(
        p1_text,
        True,
        PLAYER1_GLOW if game_state.current_player == 1 else TEXT_SECONDARY,
    )
    ...
    p2_surf = font.render(
        p2_text,
        True,
        PLAYER2_GLOW if game_state.current_player == 2 else TEXT_SECONDARY,
    )
    ...
    inst_panel = pygame.Rect(
        sw // 2 - inst_panel_width // 2,
        sh - inst_panel_height - 10,
        inst_panel_width,
        inst_panel_height,
    )
    ...
    save_rect = pygame.Rect(
        sw - btn_width * 2 - btn_spacing - 10,
        sh - btn_height - 10,
        btn_width,
        btn_height,
    )
    menu_rect = pygame.Rect(
        sw - btn_width - 10, sh - btn_height - 10, btn_width, btn_height
    )
    ...
    if status_message:
        ...
        status_panel = pygame.Rect(
            sw - status_width - 10,
            sh - btn_height - status_height - 20,
            status_width,
            status_height,
        )
...
def draw_menu() -> tuple[pygame.Rect, pygame.Rect, pygame.Rect]:
    ...
    panel_rect = pygame.Rect(
        sw // 2 - panel_width // 2, panel_y, panel_width, panel_height
    )
    ...
    for offset in [(2, 2), (-2, -2), (2, -2), (-2, 2)]:
        screen.blit(
            title_glow,
            title_glow.get_rect(center=(sw // 2 + offset[0], title_y + offset[1])),
        )
    ...
    pygame.draw.line(
        screen, PANEL_BORDER, (sw // 2 - 150, line_y), (sw // 2 + 150, line_y), 2
    )
    ...
    pve_rect = pygame.Rect(
        sw // 2 - btn_width // 2,
        btn_start_y + btn_height + btn_spacing,
        btn_width,
        btn_height,
    )
    load_rect = pygame.Rect(
        sw // 2 - btn_width // 2,
        btn_start_y + (btn_height + btn_spacing) * 2,
        btn_width,
        btn_height,
    )
    ...
    mouse_pos = pygame.mouse.get_pos()
    draw_button(
        "Player vs Player", pvp_rect, pvp_rect.collidepoint(mouse_pos), style="primary"
    )
    draw_button(
        "Player vs AI", pve_rect, pve_rect.collidepoint(mouse_pos), style="primary"
    )
    ...
    if available_space > 120:
        ...
        pygame.draw.rect(
            inst_bg, (30, 25, 20, 180), inst_bg.get_rect(), border_radius=12
        )
...
def draw_game_over(winner: int) -> pygame.Rect:
    ...
    winner_rect = winner_surf.get_rect(
        center=(sw // 2, panel_rect.centery - panel_height // 6)
    )
    ...
    restart_rect = pygame.Rect(
        sw // 2 - btn_width // 2,
        panel_rect.centery + panel_height // 6,
        btn_width,
        btn_height,
    )
    mouse_pos = pygame.mouse.get_pos()
...
def draw_difficulty_select() -> tuple[
    pygame.Rect, pygame.Rect, pygame.Rect, pygame.Rect
]:
    ...
    easy_rect = pygame.Rect(
        sw // 2 - btn_width // 2, btn_start_y, btn_width, btn_height
    )
    ...
    back_y = min(
        hard_rect.bottom + btn_spacing + 20, panel_rect.bottom - back_btn_height - 20
    )
    back_rect = pygame.Rect(
        sw // 2 - back_btn_width // 2, back_y, back_btn_width, back_btn_height
    )
    ...
    def draw_difficulty_button(rect, label, subtitle, colors, hover):
        ...
        if hover:
            ...
            pygame.draw.rect(
                glow, (*border_color, 30), glow.get_rect(), border_radius=15
            )
        ...
        sub_rect = sub_surf.get_rect(
            center=(rect.centerx, rect.centery + rect.height // 4)
        )
    ...
    draw_difficulty_button(
        ...
        "Relaxed gameplay",
        easy_colors,
        easy_rect.collidepoint(mouse_pos),
    )
    ...
    draw_difficulty_button(
        ...
        "Balanced challenge",
        medium_colors,
        medium_rect.collidepoint(mouse_pos),
    )
    ...
    draw_difficulty_button(
        ...
        "Expert AI opponent",
        hard_colors,
        hard_rect.collidepoint(mouse_pos),
    )
...
def draw_load_screen(
    scroll_offset: int = 0,
) -> tuple[list[tuple[pygame.Rect, any]], pygame.Rect]:
    ...
    if not saves:
        ...
        empty_panel = pygame.Rect(
            sw // 2 - empty_panel_width // 2,
            sh // 2 - empty_panel_height // 2,
            empty_panel_width,
            empty_panel_height,
        )
        ...
        screen.blit(
            no_saves, no_saves.get_rect(center=(sw // 2, empty_panel.centery - 15))
        )

        hint = small_font.render(
            "Start a game and click Save to create one", True, (120, 115, 100)
        )
        ...
        for i, (filepath, display_name, timestamp) in enumerate(visible_saves):
            ...
            if hover:
                shadow = pygame.Surface((rect.width, rect.height), pygame.SRCALPHA)
                pygame.draw.rect(
                    shadow, (0, 0, 0, 40), shadow.get_rect(), border_radius=12
                )
            ...
            name_surf = font.render(
                display_name, True, TEXT_COLOR if hover else TEXT_SECONDARY
            )
            screen.blit(
                name_surf,
                (
                    rect.x + 55,
                    rect.y + button_height // 4 - name_surf.get_height() // 2,
                ),
            )
            ...
            screen.blit(
                ...
                (
                    rect.x + 55,
                    rect.y + button_height * 3 // 4 - time_surf.get_height() // 2,
                ),
            )
    ...
    back_btn_height = max(45, int(55 * sh / DEFAULT_SCREEN_HEIGHT))
    back_rect = pygame.Rect(
        sw // 2 - back_btn_width // 2,
        sh - back_btn_height - 25,
        back_btn_width,
        back_btn_height,
    )
    draw_button("Back", back_rect, back_rect.collidepoint(mouse_pos))
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.VIDEORESIZE:
                # This is for older pygame versions; newer versions handle resizing automatically
                ...
                if game_mode in [MODE_PVP, MODE_PVE] and not game_state.is_terminal():
                    ...
                    if event.key == pygame.K_w:
                        ...
                        valid_moves = (
                            []
                            if wall_mode
                            else game_state.get_valid_moves(game_state.current_player)
                        )
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                ...
                # Difficulty selection
                    ...
                    easy_rect, medium_rect, hard_rect, back_rect = (
                    ...
                    if easy_rect.collidepoint(mouse_pos):
                        ...
                        game_mode = MODE_PVE
                        game_state = GameState()
                        ai_player = load_ai_player(
                            player=2, difficulty=selected_difficulty
                        )
                        valid_moves = game_state.get_valid_moves(
                            game_state.current_player
                        )
                    ...
                    if back_rect.collidepoint(mouse_pos):
                        ...
                        for rect, filepath in save_buttons:
                            if rect.collidepoint(mouse_pos):
                                try:
                                    game_state, loaded_mode, loaded_difficulty = (
                                        load_game(filepath)
                                    )
                                    ...
                                    if game_mode == MODE_PVE and loaded_difficulty:
                                        ai_player = load_ai_player(
                                            player=2, difficulty=loaded_difficulty
                                        )
                ...
                # In-game actions
                    ...
                    if (
                        save_rect.collidepoint(mouse_pos)
                        and not game_state.is_terminal()
                    ):
                        # Save the game
                        difficulty = (
                            selected_difficulty if game_mode == MODE_PVE else None
                        )
        ...
        if (
            game_mode == MODE_PVE
            and game_state.current_player == 2
            and not game_state.is_terminal()
        ):
        ...
        if game_mode == MODE_MENU:
            ...
            draw_board(
                game_state,
                valid_moves if not wall_mode else [],
                wall_preview,
                wall_valid,
            )
            draw_ui(game_state, game_mode, wall_mode, wall_orientation, status_message)
